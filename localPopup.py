...
class ButtonSelectionApp:
    def __init__(self, root):
        ...
        for i, choice in enumerate(self.choices):
            button = tk.Button(root, text=choice, bg="gray", command=lambda i=i: self.on_choice_select(i))
            button.grid(row=2, column=i, padx=5)
            self.choice_buttons.append(button)

        self.index_selected = False
        self.true_false_button = tk.Button(root, text="Index", bg="gray", command=self.toggle_index)
    ...
    def on_choice_select(self, choice_num):
        ...
        self.choice_buttons[self.choice].config(bg="blue")
